...
#define wait_hash(event) ((((long)(event) < 0) ? ~(long)(event) : (long)(event)) % NUMQUEUES)
...
/*
 * ============================================================================
 * THREAD TIMEOUT FUNCTIONS
 * ============================================================================
 */

static void thread_timeout(void *_thread)
{
...
void thread_timeout_setup(thread_t thread)
{
    ...
    /* Initialize EEVDF scheduling entity */
    eevdf_init_sched_entity(thread);
}
...
/*
 * ============================================================================
 * THREAD WAKEUP PRIMITIVE
 * ============================================================================
 */

boolean_t thread_wakeup_prim(event_t event, boolean_t one_thread, int result)
{
    queue_t q;
    int index;
    ...
    while (!queue_end(q, (queue_entry_t)thread)) {
        ...
        if (thread->wait_event == event) {
            ...
            switch (state & TH_SCHED_STATE) {
                ...
                default:
                    panic("thread_wakeup_prim: unexpected state");
                    break;
            }
            thread_unlock(thread);
            woke = TRUE;
...
static thread_t thread_select_eevdf(processor_t myprocessor)
{
    ...
    thread = eevdf_pick_next_task(&eevdf_rq_percpu[cpu_number()]);
...
boolean_t thread_invoke(thread_t old_thread, continuation_t continuation, thread_t new_thread)
{
    ...
    thread_lock(new_thread);
    if ((old_thread->stack_privilege != current_stack()) && (continuation != thread_no_continuation)) {
        switch (new_thread->state & TH_SWAP_STATE) {
            case TH_SWAPPED:
                ...
                switch (old_thread->state) {
                    ...
                    case TH_RUN | TH_WAIT | TH_SUSP:
                        old_thread->state = (old_thread->state & ~TH_RUN) | TH_SWAPPED;
                    ...
                    default:
                        panic("thread_invoke: unexpected state");
                }
                thread_unlock(old_thread);
            after_old_thread:
        ...
        if (new_thread->state & TH_SWAPPED) {
            if ((new_thread->state & TH_SW_COMING_IN) || !stack_alloc_try(new_thread, thread_continue)) {
                thread_swapin(new_thread);
                thread_unlock(new_thread);
                counter(c_thread_invoke_misses++);
                return FALSE;
...
void recover_ras(thread_t thread)
{
#if FAST_TAS
    extern void recover_ras(void);
    /* Recovery logic for RAS */
#endif
}
...
void thread_dispatch(thread_t thread)
{
    ...
    switch (thread->state & ~TH_SWAP_STATE) {
        ...
        default:
            panic("thread_dispatch: unexpected state");
    }
    thread_unlock(thread);
}
...
void compute_priority(thread_t thread, boolean_t resched)
{
    ...
    if (thread->policy == POLICY_TIMESHARE) {
        pri = thread->priority + (thread->sched_usage >> (PRI_SHIFT + SCHED_SHIFT));
...
{
    int temp_pri;
    temp_pri = thread->priority + (thread->sched_usage >> (PRI_SHIFT + SCHED_SHIFT));
    if (temp_pri > NRQS - 1) temp_pri = NRQS - 1;
...
void update_priority(thread_t thread)
{
    ...
    if (thread->policy == POLICY_TIMESHARE && thread->depress_priority < 0) {
        temp_pri = thread->priority + (thread->sched_usage >> (PRI_SHIFT + SCHED_SHIFT));
        if (temp_pri > NRQS - 1) temp_pri = NRQS - 1;
        thread->sched_pri = temp_pri;
    }
...
/*
 * ============================================================================
 * THREAD SETRUN (EEVDF Version)
 * ============================================================================
 */

void thread_setrun(thread_t th, boolean_t may_preempt)
{
    spl_t s;
...
struct run_queue *rem_runq(thread_t th)
{
    struct run_queue *rq;
...
thread_t choose_thread(processor_t myprocessor)
{
    thread_t th;
    queue_t q;
    run_queue_t runq;
...
thread_t choose_pset_thread(processor_t myprocessor, processor_set_t pset)
{
    ...
    if (myprocessor->state == PROCESSOR_RUNNING) {
        ...
        if (myprocessor == master_processor) {
            queue_enter(&pset->idle_queue, myprocessor, processor_t, processor_queue);
        } else {
            queue_enter_first(&pset->idle_queue, myprocessor, processor_t, processor_queue);
        }
        pset->idle_count++;
    }
    pset_idle_unlock();
...
static void idle_thread_continue(void)
{
    ...
    retry:
        ...
        if (state == PROCESSOR_DISPATCHING) {
            ...
            *threadp = (volatile thread_t)THREAD_NULL;
            ...
            no_dispatch_count++;
            pset->idle_count--;
            queue_remove(&pset->idle_queue, myprocessor, processor_t, processor_queue);
            myprocessor->state = PROCESSOR_RUNNING;
            pset_idle_unlock();
            counter(c_idle_thread_block++);
            thread_block(idle_thread_continue);
        } else if ((state == PROCESSOR_ASSIGN) || (state == PROCESSOR_SHUTDOWN)) {
...
static boolean_t do_runq_scan(run_queue_t runq)
{
    spl_t s;
    queue_t q;
    thread_t thread;
    ...
    if ((count = runq->count) > 0) {
        ...
        while (count > 0) {
            ...
            while (!queue_end(q, (queue_entry_t)thread)) {
                ...
                if ((thread->state & TH_SCHED_STATE) == TH_RUN && sched_tick - thread->sched_stamp > 1) {
...
void thread_check(thread_t th, run_queue_t rq)
{
    ...
    if ((th->links.next == &rq->runq[whichq]) && (rq->runq[whichq].prev != (queue_entry_t)th))
        panic("thread_check");
}
#endif
